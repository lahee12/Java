...
public class Generic_2 {
    public static void main(String[] args) {
        ...
        Pencil pencil = (Pencil)goods2.getObject(); // Pencil 로 다운캐스팅

        // 실수로 Apple 이 담겨있는 goods1 객체에서 Pencil 을 꺼냄
        // 하지만 컴파일 에러가 발생하지않음. 이유는???
        //Object 는 모든 클래스의 부모클래스로서 모든 타입의 객체 저장 가능
        //자바의 특성상 Object 는 엄격한 (=강한) 타입체크가 이루어 지지않음
        //그러므로 컴파일시점에는 타입이 맞는지 여부를 확인할 수 없으며
        //런타임시에 실제 타입 기반으로 확인하게 됨
        Pencil pencil2 = (Pencil)goods1.getObject(); //런타임에러 발생

        //Object 를 사용하는 방법은 하니의 클래스를 사용 가능하게 하고
        //이전의 문제 (Generic_1.java)는 해결 가능하나,
